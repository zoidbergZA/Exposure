        ...
        if (GameManager.Instance.TouchInput)
        {
            if (Input.touchCount > 0)
            {
                if (Input.touches[0].phase == TouchPhase.Began)
                {
                    ...
                    input = Input.touches[0].position - (Vector2)GameManager.Instance.DrillingGame.Driller.Position;
        ...
        else
        {
            ...
            if (input.sqrMagnitude < 0.1f)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    input = Input.mousePosition - GameManager.Instance.DrillingGame.Driller.Position;
